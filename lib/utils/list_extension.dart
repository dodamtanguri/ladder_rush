...
extension ListExtension on List {
  ...
  List<T> copyReplaceAt<T>(int index, T replacement, {bool growable = true}) {
    return List.from(this, growable: growable)..[index] = replacement;
  }

  ///값에 의한 첫번재 엘리먼트 대체
  List<T> copyReplaceWith<T>(T value, T replacement, {bool growable = true}) =>
  ...
  List<T> copyRemoveAt<T>(int index, {bool growable = true}) =>
      List.from(this, growable: growable)..removeAt(index);
  ...
  List<T> copyRemoveAll<T>(T value, {bool growable = true}) =>
      List.from(where((element) => element != value), growable: growable);

  ///값에 의한 모든 엘리먼트 추가
  List<T> copyAddAll<T>(List<T> add, {bool growable = true}) =>
      List.from(this, growable: growable)..addAll(add);
}
